...
def ka(S, Ks):
    ...
    if nd == 2:
        return rS(
            [signal.convolve(I, K)[: d[0], : d[1]] for I, K in zip(Is, Ks)], allI=False
        )
    if nd == 3:
        return rS(
            [signal.convolve(I, K)[: d[0], : d[1], : d[2]] for I, K in zip(Is, Ks)],
            allI=False,
        )
...
def rCs(S, allI=False, op=0, f=False, spec=False):
    ...
    if op == 0:
        ...
        return np.array(
            [
                rC(Is[a], Is[b], f=f, spec=spec)
                for a in range(nph)
                for b in range(a, nph)
            ]
        )
    # All
    if op == 1:
...
def Ceq_search(d, nev, b=True, ref=True, f=False, spec=False, tol=1e-13, info=False):
    ...
    def unrelloc(i1, i2):
        return not rel(Ss[i1], Ss[i2], ref)

    while i < nSs:
        if info:
            print(
                "...Searching for cases - %i / %i = %.4f" % (i + 1, nSs, (i + 1) / nSs)
            )
        Ci = C[i]
        if f:
    ...
    if len(out) > 1:
        ...
        def unrelloc(i1, i2):
            return not rel(out[i1][0], out[i2][0], ref)
...
def Ceq_search_save(d, nev, folder="default", info=False):
    if folder == "default":
        folder = (
            ...
            + str(len(d))
            + "d_"
            + "_".join([str(dd) for dd in d])
            + "_nev_"
            + "_".join([str(dd) for dd in nev])
        )
...
def plot_S(S, per=False, fs=(15, 15), save="", title="", show=True):
    ...
    if ndim == 2:
        plt.figure()
        if not per:
            plt.matshow(S, cmap=plt.get_cmap(colormap, nph), vmin=0.5, vmax=nph + 0.5)
        else:
            plt.matshow(
                periodic(S), cmap=plt.get_cmap(colormap, nph), vmin=0.5, vmax=nph + 0.5
            )
        plt.colorbar(ticks=np.arange(1, nph + 1))
        if len(save) > 0:
            plt.savefig("./" + save + ".png")
    if ndim == 3:
        ...
        for i in range(1, nph):
            mylegend.append(
                Patch(label=str(i), facecolor=colors[i - 1], edgecolor="black")
            )
        mylegend.append(Patch(label=str(nph), facecolor="white", edgecolor="black"))
